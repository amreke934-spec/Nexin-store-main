...
import { Home, Wallet, PackageCheck, User, LogIn } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav
      ...
      className="fixed bottom-3 sm:bottom-4 z-40 w-[94%] max-w-md glass-nav rounded-2xl sm:rounded-3xl p-1.5 sm:p-2 shadow-2xl border border-slate-200/90 dark:border-slate-800/90 backdrop-blur-xl"
    >
      <div className="grid grid-cols-4 gap-1 sm:gap-2 items-center">
        ...
        >
          ...
          <span className="text-[10px] sm:text-xs mt-1 font-medium tracking-tight">المحفظة</span>
          {activeTab === 'wallet' && (
            <span className="absolute -bottom-0.5 w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
          )}
        </button>
