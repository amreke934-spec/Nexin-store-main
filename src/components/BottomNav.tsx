...
import { Home, Package, User, LogIn, Lock, Wallet } from 'lucide-react';

interface BottomNavProps {
  activeTab: 'products' | 'orders' | 'track' | 'settings' | 'history' | 'auth' | 'admin' | 'deposit' | string;
    ...
    <>
      ...
      >
        <div className="grid grid-cols-4 gap-1 sm:gap-1.5 items-center">
          ...
          {/* 3. Deposit / Wallet (Next to My Account) */}
          <button
          ...
          >
            ...
            {activeTab === 'deposit' && !isMaintenanceLocked && (
              <span className="absolute -bottom-0.5 w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
            )}
          </button>

          {/* 4. User Account / Settings / Auth */}
          <button
